...
function uploadifySuccess(file, data, response){
	...
	if ( data['status'] ) {
        $( "input[name='" + data['formName'] + "']" ).val( data['info'] );
        $( "." + data['formName']).html( '<img src="' + data['info'] + '" height="100" />' );
    } else {
        alertMsg.error( '错误提示：' + data['info'] );
    }
}

/**
 * http://www.uploadify.com/documentation/uploadify/onuploadstart/
 * 限制上传个数
 */
function uploadStart() {
    window.uploadifyTotal = 0;
    window.uploadifyLimit = 1;
}

/**
 * http://www.uploadify.com/documentation/uploadify/onuploaderror/
 * 上传失败后执行的回调函数，从服务商返回数据到前端
 */
