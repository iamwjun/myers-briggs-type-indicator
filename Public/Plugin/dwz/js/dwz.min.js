...
var DWZ = {
    ...
    ajaxError: function(xhr, ajaxOptions, thrownError){
        if (alertMsg) {
            alertMsg.error("Http status: " + xhr.status + " " + xhr.statusText + "</p>");
        }
    ...
    $.fn.extend({
        ...
        layoutH: function($refBox){
            return this.each(function(){
                var $this = $(this);
                if (!$refBox) 
                    $refBox = ("dialog" == $this.attr("layoutType") && $.pdialog) ? $.pdialog.getCurrent().find(".dialogContent") : $("#container .tabsPageContent");
...
(function($){
    $.extend($.fn, {
        ...
        _check: function(op){
            ...
            ckbox.append("<input type='checkbox' name='" + $input.attr("tname") + "' value='" + $input.attr("tvalue") + "' style='display:none;'/>").click(function(){
...
function initLayout(){
    ...
    $("#container .tabsPageContent").height("auto").find("[layoutH]").layoutH();
...
function initUI(_box){
    ...
    if ($.fn.xheditor) {
        $("textarea.editor", jParent).each(function(){
            ...
            $this.xheditor({
                ...
                upLinkUrl: APP + "/ask/upload/",
                upLinkExt: "zip,rar,txt,7z,doc,docx,xls,ppt,chm",
                upImgUrl: APP + "/ask/upload/",
                upImgExt: "jpg,jpeg,gif,png",
                upFlashUrl: APP + "/ask/upload/",
                upFlashExt: "swf",
    ...
    //target=selecttodo
    $("a[target=selectedTodo]", jParent).each(function(){
        var $this = $(this);
        ...
        $this.click(function(){
            ...
            function _doPost(){
                $.ajax({
                    ...
                    data: function(){
                        if (postType == 'map') {
                            return $.map(id.split(','), function(val, i){
                                return {
                                    name: selectedIds,
                                    value: val
                                };
    ...
    $("a[target=selected]", jParent).each(function(){
        var $this = $(this);
        var selectedIds = $this.attr("rel") || "ids";
        var postType = $this.attr("postType") || "map";
        ...
        $(this).click(function(event){
            ...
            var url = unescape($this.attr("href")).replaceTmById(jParent) + "id/" + id;
            DWZ.debug(url);
            if (!url.isFinishedTm()) {
                alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
                return false;
                ...
                jTabs.each(function(iTabIndex){
                    ...
                    $("a", this).each(function(){
                        if ($(this).hasClass(op.ajaxClass)) {
                            ...
                            if ($(this).hasClass(op.closeClass)) {
                                $(this).click(function(event){
            ...
            return this.each(function(){
                ...
                setTimeout(function(){
                    ths.each(function(i){
                        var $th = $(this);
                        var style = aStyles[i];
                        $th.addClass(style[1]).removeAttr("align").hoverClass("hover").removeAttr("width").width(style[0]);
                    });
                }, 1);
...
function iframeCallback(form, callback){
    // 判断座机号码
    if (form.plane_number) {
        val = form.plane_number.value;
        reg = /^0\d{2,3}-\d{7,8}$/;
        if (!reg.test(val)) {
            alert("请填写座机号码");
            form.plane_number.focus();
            return false;
        }
    }
    if (!$(form).valid()) {
...
function _getPagerForm($parent, args){
    ...
    if (form) {
        args = args || {};
        if (args["pageNum"]) 
            form.pageNum.value = args["pageNum"];
        if (args["numPerPage"]) 
            form.numPerPage.value = args["numPerPage"];
        if (args["orderField"]) 
...
(function($){
    ...
    $.extend(Datepicker.prototype, {
        ...
        getDateWrap: function(date){
            if (!date) 
                date = this.sDate ? this.parseDate(this.sDate) : new Date();
    ...
    $.extend($.fn, {
        jPanel: function(options){
            ...
            return this.each(function(){
                ...
                if (collapse) 
                    $("<a href=\"\"></a>").addClass(close ? op.exp : op.coll).insertAfter(title);
