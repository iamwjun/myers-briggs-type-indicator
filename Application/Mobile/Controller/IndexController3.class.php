        ...
        //这是在微信浏览器中的处理
         if($page<2)
              ...
              if (!isset($_GET['code'])){
                if($ss==1)
                {
                    $path="https://www.zug365.cn/Mobile/Index/question.html?sex=1&page=".$page."&score=".$fenshu;
                }
                else{
                    $path="https://www.zug365.cn/Mobile/Index/question.html?sex=2&page=".$page."&score=".$fenshu;
                }
	...
	public function question_xgcs()
	{
         ...
         if($page<2)
        {
              if (!isset($_GET['code']))
              {
                if($ss==1)
                {
                    $path="https://www.zug365.cn/Mobile/Index/question_xgcs.html?type=".$tp."&page=".$page."&score=".$fenshu;
                }
                else{
                    $path="https://www.zug365.cn/Mobile/Index/question_xgcs.html?type=".$tp."&page=".$page."&score=".$fenshu;
                }
	   ...
	   $score=I('get.score');
		...
		if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {
           ...
           if (!isset($_GET['code'])){
                if($ss==1)
                {
                    $path="https://www.zug365.cn/Mobile/Index/question_zscs.html?type=".$tp."&page=".$page."&score=".$fenshu;
                }
                else{
                    $path="https://www.zug365.cn/Mobile/Index/question_zscs.html?type=".$tp."&page=".$page."&score=".$fenshu;
                }
   ...
   public function zstest()
	...
	public function payOrder(){
        ...
        $data = array(
            ...
            'notify_url' => 'http://www.zug365.cn/mobile/Index/notify_pay/money/'.$pay_money."/sex/".$sex."/score/".$score,
            'trade_type' => 'JSAPI',
            'openid' => $openid
        );
    ...
    public function payOrder_zs(){
        ...
        $data = array(
            ...
            'total_fee' => $paymoney,
            'spbill_create_ip' => '127.0.0.1',
            'notify_url' => 'http://www.zug365.cn/mobile/Index/notify_pay_zs/money/'.$pay_money."/type/".$type."/score/".$score,
            'trade_type' => 'JSAPI',
            'openid' => $openid
        );
        $paySign = $this->makeWxPaySign($data, 'kjag293sakgsa089346a932jas6kja93');
    ...
    public function h5_payOrder(){
        ...
        $out_trade_no=time();
         ...
         if($category==1)
         {
          ...
          $path="&redirect_url=https://www.zug365.cn/Mobile/Index/question/sex/".$sex."/page/".$page."/score/".$score."/out_trade_no/".$out_trade_no."/category/1";
         }
         if($category==2)
         {
           //$path="&redirect_url=http://www.zug365.cn/Mobile/Index/search_xg_res/type/1/score/".$score;
          $path="&redirect_url=https://www.zug365.cn/Mobile/Index/question_xgcs/type/1/page/".$page."/score/".$score."/out_trade_no/".$out_trade_no."/category/2";
         }
         if($category==3)
         {
           //$path="&redirect_url=http://www.zug365.cn/Mobile/Index/search_zs_res/type/1/score/".$score;
          $path="&redirect_url=https://www.zug365.cn/Mobile/Index/question_zscs/type/1/page/".$page."/score/".$score."/out_trade_no/".$out_trade_no."/category/3";
         }
        ...
        $data = array(
            ...
            'notify_url' => "http://www.zug365.cn/mobile/Index/notify_h5pay/category/".$category."/score/".$score."/sex/".$sex,
            'trade_type' => 'MWEB',
    ...
    //抖音API回传
    public function douyin_api()
    {
        $data = array(
          'event_type' => 'active',
          'event_weight' => '0.561',
          'biz_channel' => 5,
          'context' => '{"device":{"platform":"ios","idfa":"FCD369C3-F622-43B8-AFDE-12012349F35B"},"ad":{"ad_id":"16521212134"}},timestamp:"1604299664"',
        );
        $wxRes =$this->post('https://analytics.oceanengine.com/api/v2/valuable',$data);
