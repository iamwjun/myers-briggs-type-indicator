...
<head>
	...
	<title>{$title}-{$config.web_title}</title>
...
<body>
	...
	<header class="mui-bar mui-bar-nav" style="background:#e64343">
		...
		<h1 class="mui-title" style="color:#FFF"><if condition="$title neq ''">{$title}<else />全部更新</if></h1>
	...
	<div class="apply_repair">
		...
		<ul>
          <volist name="datalist" id="vo">
            <a href="{:U('Index/info',array('id'=>$vo['id']))}" style="color:#444343">
	            <li style="font-size:13px; padding:22px 0px 22px 10px; border-bottom:1px dashed lightgray">
	            <div style="width:260px; display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{$vo.title}</div>
	            <span style="font-size:13px; color:#a9a7a7; float:right; margin:-20px 6px 0px 0px">{$vo.create_time|date='Y-m-d',###}</span>
	            </li>
            </a>
          </volist>
		</ul>
