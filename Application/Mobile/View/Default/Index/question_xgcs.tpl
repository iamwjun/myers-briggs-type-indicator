...
<title>情感、性格 - 问答测试</title>
    ...
    <script type="text/javascript">    
	 ...
	 function chioce(score,obj)
	 {
       ...
       $('.ans_1').css({"background":"#f3f1f1","color":"#000"});
       $('.ans_2').css({"background":"#f3f1f1","color":"#000"});
       ...
       if(page<cont)
       {
	       ...
	       window.location="{:U('Index/question_xgcs')}?type="+type+"&page="+turn+"&score="+total_score+"&open_id="+open_id;
	   }
	   else{
     ...
     function show_result()
      {
        ...
        $.ajax({
             ...
             data:"openid="+openid+"&money=29.9&type=1&score="+score,
             ...
             var strs=JSON.parse(date);
                     WeixinJSBridge.invoke(
                      ...
                      if(res.err_msg == "get_brand_wcpay_request:ok" ){
                        //alert('支付成功！');
                        window.location="{:U('Index/notify_pay')}?sex="+sex+"&score="+score;
                      } 
                      else{
                        alert('支付失败！');
                      }
      ...
      function h5_pay()
      {
        ...
        $.ajax({
             ...
             data:"money=0.01&sex="+sex+"&score="+score+"&category=2&page="+page,
             success:function(date){
...
<body style="overflow-x:hidden;width:100%;padding:0;margin:0;">
  ...
  <div class="main" style="width:100%;height:1240px;">
     ...
     <div style="width:80%;height:500px;background:#FFF;border-radius:10px;position:absolute;top:34%;left:11%">
        ...
        <div class="title" style="width:90%;height:80px;float:left;padding-left:5%;font-size:26px;color:#000;font-weight:bold;line-height:32px">
           {$jg_list[0]['title']}
        </div>
        <div class="ans_1" style="width:79%;height:auto;float:left;background: #f3f1f1;margin:8% 0px 0px 6%;border-radius:10px;line-height:40px;padding:10px 10px 10px 5%;font-size:28px;color:#000;font-weight:bold" onclick="chioce(1,this)">
          A&nbsp;{$jg_list[0]['intro']}
        </div>
        <div class="ans_2" style="width:79%;height:auto;float:left;background: #f3f1f1;margin:8% 0px 0px 6%;border-radius:10px;line-height:40px;padding:10px 10px 10px 5%;font-size:28px;color:#000;font-weight:bold" onclick="chioce(2,this)">
          B&nbsp;{$jg_list[0]['content']}
        </div>
     </div>
     ...
     <div id="next_tm" style="width:120px;height:40px;background:#FFF;border-radius:10px;font-size:28px;color:#7073e8;text-align:center;line-height:40px;font-weight:bold;padding:12px 40px 12px 40px;position:absolute;top:74%;left:58%" onclick="next()">下一题>></div>
      ...
      <div id="pay" style="height:40px;background:#FFF;border-radius:10px;font-size:26px;color:#7073e8;text-align:center;line-height:40px;font-weight:bold;padding:16px 26px 16px 20px;position:absolute;top:85%;left:26%;display:none" onclick="show_result()">点击支付29.9元查看测试结果</div>
     
     <div id="h5_pay" style="height:40px;background:rgb(207, 53, 53);color:#FFF;border-radius:10px;font-size:26px;text-align:center;line-height:40px;font-weight:bold;padding:16px 26px 16px 20px;position:absolute;top:85%;left:26%;display:none" onclick="h5_pay()">点击支付29.9元查看测试结果</div>
