    ...
    <div class="pd-20">
        ...
        <div class="text-c">
            <form action="" method="post">
                <!-- 用户名 -->
                <input type="text" name="title" value="{$search['title']}" placeholder="输入菜单名称" class="input-text radius width-200" />
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                <a href="javascript:void(0);" onclick="layer_full('添加菜单','__URL__/add/pid/16','','');" class="btn btn-primary radius">
                            ...
                            <td>
								<if condition="$volist['picture'] neq ''">
									<a href="__ROOT__/{$volist['picture']}" rel="image-viewer" title="{$volist['title']}">
										<img src="__ROOT__/{$volist['picture']}" height="50" width="50" class="radius" />
									</a>
								<else />
									无
								</if>
							</td>
                            <td>{$volist['btn_link']}</td>
