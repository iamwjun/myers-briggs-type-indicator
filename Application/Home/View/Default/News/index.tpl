...
<head>
	...
	<title>新闻动态-{$config.web_title}</title>
    ...
    <style>
     ...
     .news_box_body{height: 40px; overflow: hidden; font-size: 14px; color: #999;}
    </style>
    <!--[if lt ie 9]> 
...
<body>
	...
	<div class="news">
		<div class="news_tab">
			<ul>
			<foreach name="category" item="vo" key="i">
              <li <?php if($i==0){ ?>class="active"<?php }?>><span><?=$vo['title']?></span></li>
			</foreach>
			</ul>
		</div>
		<div class="news_box">
			<ul>
			<volist name="list_gs" id="vo">
				<a href="{:U('News/info',array('id'=>$vo['id']))}"><li>
					<h5 style="font-weight: bold;color: #666">{$vo.title}</h5>
